...
function resetarFormulario() {
    ...
    document.getElementById('inputNome').value = ""; document.getElementById('inputSeries').value = "";
    document.getElementById('inputReps').value = ""; document.getElementById('inputMin').value = "";
    document.getElementById('inputSec').value = ""; document.getElementById('checkTipoTempo').checked = false;
    toggleTipoInputs();
...
function renderizarListaEditor() {
    const div = document.getElementById('listaEditor'); div.innerHTML = "";
    ...
    rotinaCache.forEach((ex, idx) => {
        if(ex.tipo === 'biset') {
            div.innerHTML += `
                <div class="card" style="display:flex; justify-content:space-between; align-items:center; border-left:4px solid #9c27b0;">
                    <div style="flex:1"><span style="color:#9c27b0; font-weight:bold; font-size:0.8em;">BI-SET</span><div style="color:#fff;">1. ${ex.ex1.nome} (${ex.ex1.reps})</div><div style="color:#fff;">2. ${ex.ex2.nome} (${ex.ex2.reps})</div></div>
                    <div class="card-actions"><button class="btn-action edit" onclick="editarItem(${idx})">${iconEdit}</button><button class="btn-action delete" onclick="removerItem(${idx})">${iconTrash}</button></div>
                </div>`;
        } else {
            let info = ex.tipo === 'timer' ? formatTime(ex.tempo) : `${ex.reps} Reps`;
            div.innerHTML += `
                <div class="card" style="display:flex; justify-content:space-between; align-items:center;">
                    <div style="flex:1"><strong style="color:#fff;">${ex.nome}</strong><br><small style="color:#666;">${ex.series} x ${info}</small></div>
                    <div class="card-actions"><button class="btn-action edit" onclick="editarItem(${idx})">${iconEdit}</button><button class="btn-action delete" onclick="removerItem(${idx})">${iconTrash}</button></div>
                </div>`;
        }
    });
...
function editarItem(idx) {
    const item = rotinaCache[idx]; indiceEdicao = idx;
    document.getElementById('telaEditor').scrollIntoView({ behavior: 'smooth' });
    if (item.tipo === 'biset') {
        mudarAbaEditor('biset');
        document.getElementById('biNome1').value = item.ex1.nome; document.getElementById('biReps1').value = item.ex1.reps;
        document.getElementById('biNome2').value = item.ex2.nome; document.getElementById('biReps2').value = item.ex2.reps;
        document.getElementById('biSeries').value = item.series; document.getElementById('btnAddBiset').innerText = "Salvar Alteração";
    } else {
        mudarAbaEditor('normal');
        document.getElementById('inputNome').value = item.nome; document.getElementById('inputSeries').value = item.series;
        const check = document.getElementById('checkTipoTempo');
        if (item.tipo === 'timer') {
            check.checked = true; toggleTipoInputs();
            const m = Math.floor(item.tempo / 60); const s = item.tempo % 60;
            document.getElementById('inputMin').value = m; document.getElementById('inputSec').value = s;
        } else {
            check.checked = false; toggleTipoInputs(); document.getElementById('inputReps').value = item.reps;
        }
...
function adicionarAoTreino(modo) {
    ...
    if(modo === 'biset') {
        ...
        novoEx.tipo='biset'; novoEx.series=parseInt(s); novoEx.ex1={nome:n1,reps:r1}; novoEx.ex2={nome:n2,reps:r2};
    } else {
        ...
        if(t) {
            ...
            const r=document.getElementById('inputReps').value; if(!r) return alert("Preencha repetições.");
            novoEx.tipo='reps'; novoEx.reps=parseInt(r);
        }
    }
    if (indiceEdicao !== null) rotinaCache[indiceEdicao] = novoEx; else rotinaCache.push(novoEx);
    resetarFormulario(); renderizarListaEditor();
}
function removerItem(idx) { if(confirm("Remover?")) { if (indiceEdicao === idx) resetarFormulario(); rotinaCache.splice(idx,1); renderizarListaEditor(); } }
...
function atualizarCardTreino(progresso) {
    ...
    const serieInfo = `${seriesFeitas + 1} / ${exAtual.series}`;
    ...
    if(exAtual.tipo === 'biset') {
        ...
        div.innerHTML = `
            <div style="text-align:center;">
                <span class="exec-badge">SÉRIE ${serieInfo}</span>
                <div class="biset-row" style="margin-top:20px;">
                    <div class="biset-item">
                        <div class="neon-circle purple">
                            <div class="circle-content">
                                <div class="neon-number" style="font-size:3.5em;">${exAtual.ex1.reps}</div>
                                <span class="circle-label" style="font-size:0.7em;">${exAtual.ex1.nome}</span>
                            </div>
                        </div>
                        <div class="input-giant-wrapper"><input type="number" id="inputCarga1" class="input-giant" value="${c1}" placeholder="0"><span class="input-label">KG</span></div>
                    </div>
                    <div class="biset-item">
                        <div class="neon-circle purple">
                            <div class="circle-content">
                                <div class="neon-number" style="font-size:3.5em;">${exAtual.ex2.reps}</div>
                                <span class="circle-label" style="font-size:0.7em;">${exAtual.ex2.nome}</span>
                            </div>
                        </div>
                        <div class="input-giant-wrapper"><input type="number" id="inputCarga2" class="input-giant" value="${c2}" placeholder="0"><span class="input-label">KG</span></div>
                    </div>
                </div>
                <br>
        ...
        div.innerHTML = `
            <div style="text-align:center;">
                <span class="exec-badge">SÉRIE ${serieInfo}</span>
                <h2 class="exec-title" style="color:#00aaff">${exAtual.nome}</h2>
                <div class="neon-circle blue">
                    <div class="circle-content">
                        <div class="neon-number">${formatTime(exAtual.tempo)}</div>
                        <span class="circle-label">TEMPO</span>
                    </div>
                </div>
                <input type="number" id="inputTempoExec" value="${exAtual.tempo}" style="display:none">
                <button class="btn-neon blue full" onclick="iniciarExecucaoCardio('${exAtual.id}', ${seriesFeitas})">Iniciar Timer</button>
            </div>`;
    } else {
        ...
        div.innerHTML = `
            <div style="text-align:center;">
                <span class="exec-badge">SÉRIE ${serieInfo}</span>
                <h2 class="exec-title">${exAtual.nome}</h2>
                <div class="neon-circle green">
                    <div class="circle-content">
                        <div class="neon-number">${exAtual.reps}</div>
                        <span class="circle-label">REPETIÇÕES</span>
                    </div>
                </div>
                <div class="input-giant-wrapper"><input type="number" id="inputCarga" class="input-giant" value="${c}" placeholder="0"><span class="input-label">CARGA (KG)</span></div>
                <br>
...
function iniciarExecucaoCardio(id, feitas) {
    ...
    document.getElementById('timerDisplay').innerText=formatTime(t);
